...
class EdgeBuilder(Model):
    ...
    def __init__(self,
        ...
        self._predicate_mention_scorer = TimeDistributed(torch.nn.Linear(predicate_mention_feedforward.get_output_dim(), 1))
    ...
    def forward(self,  # type: ignore
        ...
        top_predicate_span_embeddings, \
            ...
            top_predicate_spans, \
            top_predicate_span_scores = self._prune_spans(spans, span_mask, predicate_span_mentions, predicate_span_scores, num_predicate_spans_to_keep)
        
        top_role_span_embeddings, \
            top_role_span_masks, \
            top_role_span_indices, \
            top_role_spans, \
            top_role_span_scores = self._prune_spans(spans, span_mask, role_span_mentions, role_span_scores, num_role_spans_to_keep)
    ...
    def compute_relation_representations(self,  # type: ignore
        ...
        p2p_edge_pairwise_embeddings = self._compute_span_pair_embeddings(top_predicate_span_embeddings, top_predicate_span_embeddings)
        p2r_edge_pairwise_embeddings = self._compute_span_pair_embeddings(top_predicate_span_embeddings, top_role_span_embeddings)   
        ...
        p2p_edge_scores += (top_predicate_mention_scores.unsqueeze(-1) + top_predicate_mention_scores.transpose(1, 2).unsqueeze(-1))
        p2r_edge_scores += (top_predicate_mention_scores.unsqueeze(-1) + top_role_mention_scores.transpose(1, 2).unsqueeze(-1))
    ...
    def predict_labels(self, output_edges, output_nodes, lu_frame_map, frame_fe_map, metadata, p2p_edges_labels=None, p2r_edges_labels=None):
    ...
    def _decode_frames(self, output_edges, output_nodes, lu_frame_map, metadata_list):
        ...
        for node_types, node_attr_scores, p2p_edges, metadata in zip(decoded_node_types_dict, decoded_node_attr_scores_dict, decoded_p2p_edges_dict, metadata_list):
            frames = dict()
            cspan2frames = dict()
            dspan2frames = dict()
            ...
            for node_ix, node_type in node_types.items():
                ...
                if "PPRD" in node_type:
                    ...
                    if frame_ix not in frames and len(frame_ix) >= 2 and is_clique(frame_ix, p2p_edges):
                        frame_label = get_constrained_frame_label(sentence_tokens, lemma_tokens, frame_ix, frame_score, lu_frame_map, \
                                                                  self._node_attr_vocab, self._id_to_node_attr_vocab, self._num_node_attr)
                        if frame_label == "O":
                            continue
        ...
        for top_role_spans, top_predicate_spans, predicted_p2r_edge_scores, \
            ...
            for i, j in itertools.product(range(predicate_keep), range(role_keep)):
                ...
                if span_1 in predicted_cspan2frames:
                    ...
                    valid_frame_elements = torch.LongTensor([self._p2r_edges_vocab[fe] + 1 for fe in frame_fe_map[frame_label]  if fe in self._p2r_edges_vocab])
            ...
            for (frame_ix, frame_label), sub_roles_dict in conflict_frame_elements.items():
                ...
                for role_span, role_scores in sub_roles_dict.items():
                    frame_element_label_id = torch.argmax(sum(role_scores) + torch.log(valid_frame_elements_mask)).item()-1
                    if frame_element_label_id < 0:
                        continue
                    frame_element_label = self._id_to_p2r_edges_vocab[frame_element_label_id]
    ...
    @staticmethod
    def _get_pruned_gold_relations(relation_labels, top_right_span_indices, top_right_span_masks, top_left_span_indices, top_left_span_masks):
